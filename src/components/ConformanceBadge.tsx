...
const PRESENTATION: Record<Verdict, { icon: string; label: string; cls: string }> = {
 ...
 */
export function ConformanceBadge({ conformance }: { conformance: Conformance }) {
  const p = PRESENTATION[conformance.verdict]
  const count = conformance.findings.length
  const tooltip =
    count === 0
      ? 'No conformance issues'
      : conformance.findings.map((f) => `${f.severity}: ${f.message}`).join('\n')
