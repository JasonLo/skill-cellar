...
import { discover, type SkillDescriptor } from '../core/fs-skills'
...
function loadData(): Data {
  ...
  return { rows, projectCount: project.length, globalCount: global.length }
}
  ...
  if (sortedRows.length === 0) {
    return (
      <box flexDirection="column">
        ...
        <text fg="#888888">
          No skills installed in project ({projectSkillsRoot()}) or global (
          {globalSkillsRoot()}).
        </text>
...
function sortRows(rows: Row[], sort: Sort): Row[] {
  const groups: Record<'project' | 'global', Row[]> = {
    project: [],
    global: [],
  }
  ...
  return [...groups.project, ...groups.global]
}
